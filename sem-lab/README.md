...
```
cd backend
npm install express cors cookie-parser multer @prisma/client prisma
npx prisma generate --schema=./schema.prisma
# set DATABASE_URL in your environment, then:
npx prisma db push --schema=./schema.prisma
node -e "require('./server.js').seedUseCases()"   # seeds the 12 use cases
```
Then wire up an actual HTTP listener (`app.listen(4001)`) and real
authentication before running this anywhere beyond local testing — the
current `authenticate` middleware is a development placeholder.
